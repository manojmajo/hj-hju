...
font2 = pygame.font.SysFont("comicsansms", 50, True)
...
class labyrinth:
    ...
    def __init__(self,id):
        ...
        for j in range(0,636,33):
            for i in range(0,851,33):
...
running = True
while running:
    screen = pygame.display.set_mode((950, 833))
    done = False
    ...
    id += 1
    maze = labyrinth(id)
    goal = pygame.Rect(899,401,25,25)
    victory = False
    ...
    while not done:
        ...
        if not victory and not pause:
            ...
            screen.blit(text[0], (750, 15))
        
        # victory screen
        if victory:
            screen.fill((0, 0, 0))
            time_text = font1.render("Time Taken: " + text[1],True,(250,250,250))
            victory_text = font2.render("Victory your score is 100!",True,(250,250,250))
            reset = font3.render("(Press Enter to Start New Game)",True,(250,250,250))

            screen.blit(victory_text,(468 - (victory_text.get_width() // 2), 348 - (victory_text.get_height() // 2)))
            screen.blit(time_text, (468 - (time_text.get_width() // 2), (348 - (time_text.get_height() // 2)) + victory_text.get_height()))
            screen.blit(reset, (468 - (reset.get_width() // 2), (348 - (reset.get_height() // 2)) + victory_text.get_height() + time_text.get_height()))
